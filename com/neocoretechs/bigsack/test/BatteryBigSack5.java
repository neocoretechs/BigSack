...
public class BatteryBigSack5 {
	...
	*/
	 ...
	 * @throws Exception
	 */
	public static void battery1(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
	 ...
	 * @throws Exception
	 */
	public static void battery1A(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		for(int i = min; i < max; i++) {
	 ...
	 * @throws Exception
	 */
	public static void battery1A1(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();	
	 ...
	 * @throws Exception
	 */
	public static void battery1B(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		Stack s = new Stack();
	 ...
	 * @throws Exception
	 */
	public static void battery1C(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
	 ...
	 * @throws Exception
	 */
	public static void battery1D(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
	 ...
	 * @throws Exception
	 */
	public static void battery1D1(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		// headset strictly less than 'to' element
	...
	/**
	 * headset returns values strictly less than 'to' element, here we dont find exact key
	 * We are checking VALUE returns against expected key from KV iterator
	 * @param session
	 * @param argv
	 * @throws Exception
	 */
	public static void battery1D2(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		// headset strictly less than 'to' element
		Iterator<?> itk = session.headMapKV(key+String.format(uniqKeyFmt, max+10)); // set is strictly less than 'to' element so we use max val
		//System.out.println("BATTERY1D1 iterator:"+itk);
		if( itk == null)
			throw new Exception("BATTERY1D2 FAIL iterator for K/V headmap came back null for "+key+String.format(uniqKeyFmt, max+10));
		int ctr = 0;
		while(itk.hasNext()) {
			KeyValuePair f = (KeyValuePair) itk.next();
			if( f == null)
				throw new Exception("BATTERY1D2 FAIL K/V pair came back null for iterator.next() for "+key+String.format(uniqKeyFmt, max+10));
			//System.out.println("BATTERY1D1 iterator result:"+f);
			String nval = val + String.format(uniqKeyFmt, ctr);
			if( !f.value.equals(nval) ) {
				 System.out.println("BATTERY1D2 FAIL "+f+" -- "+nval);
				 System.out.println("BATTERY1D2 FAIL counter reached "+ctr);
				throw new Exception("B1D2 Fail on get with "+f+" -- "+nval);
			}
			++ctr;
		}
		if( ctr != max+1 ) {
			 System.out.println("BATTERY1D2 FAIL counter reached "+ctr+" not "+(max+1));
			throw new Exception("B1D2 FAIL counter reached "+ctr+" not "+(max+1));
		}
		 System.out.println("BATTERY1D2 SUCCESS in "+(System.currentTimeMillis()-tims)+" ms.");
	}
	 ...
	 * @throws Exception
	 */
	public static void battery1E(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
	 ...
	 * @throws Exception
	 */
	public static void battery1E1(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		// from element inclusive to element exclusive
		// notice how we use base key to set lower bound as the partial unformed key is least possible value
	...
	/**
	 * Subset returns persistent collection iterator 'from' element inclusive, 'to' element exclusive
	 * where keys are not located
	 * @param session
	 * @param argv
	 * @throws Exception
	 */
	public static void battery1E2(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		// from element inclusive to element exclusive
		// notice how we use base key to set lower bound as the partial unformed key is least possible value
		int minx = min+100;
		String sminx =  key+String.format(uniqKeyFmt, minx-1)+"a";
		int maxx = max-100;
		String smaxx =  key+String.format(uniqKeyFmt, maxx)+"z";
		Iterator<?> itk = session.subMap(sminx, smaxx); // 'to' exclusive so we use max val
		int ctr = minx;
		while(itk.hasNext()) {
			Object f = itk.next();
			...
			//System.out.println(nval);
			if( !f.equals(nval) ) {
				 System.out.println("BATTERY1E2 FAIL retrieved:"+f+" -- expected:"+nval);
				 System.out.println("BATTERY1E2 FAIL counter reached "+ctr+" not "+maxx);
				throw new Exception("B1E2 Fail on get with retrieved:"+f+" -- expected:"+nval);
			}
			if(ctr == maxx)
				break;
			++ctr;
		}
		 System.out.println("BATTERY1E2 SUCCESS in "+(System.currentTimeMillis()-tims)+" ms.");
	}
	/**
	 * Subset returns persistent collection iterator 'from' element inclusive, 'to' element exclusive
	 * We are checking VALUE returns against expected key from KV iterator
	 * where values are not explicitly located
	 * @param session
	 * @param argv
	 * @throws Exception
	 */
	public static void battery1E3(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		// from element inclusive to element exclusive
		// notice how we use base key to set lower bound as the partial unformed key is least possible value
		// 'to' exclusive so we use max val
		...
		while(itk.hasNext()) {
			...
			//System.out.println(nval);
			if( !f.key.equals(nval) || !f.value.equals(nvalx)) {
				 System.out.println("BATTERY1E3 FAIL retrieved:"+f+" -- expected:"+nval);
				 System.out.println("BATTERY1E3 FAIL counter reached "+ctr);
				throw new Exception("B1E3 Fail on get with retrieved:"+f+" -- expected:"+nval);
			}
			if(ctr == maxx)
				break;
			++ctr;
		}
		 System.out.println("BATTERY1E3 SUCCESS in "+(System.currentTimeMillis()-tims)+" ms.");
	}
	/**
	 ...
	 * @throws Exception
	 */
	public static void battery1F(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
	 ...
	 * @throws Exception
	 */
	public static void battery1F1(TransactionalTreeMap session, String[] argv) throws Exception {
		long tims = System.currentTimeMillis();
		// subset strictly less than 'to' element
