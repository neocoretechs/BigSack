...
public class BufferPool {
	 ...
	 * @throws IOException
	 */
	public synchronized void createPool(GlobalDBIO globalIO, IoInterface ioWorker, int tablespace) throws IOException {
		blockBuffer[tablespace] = new MappedBlockBuffer(ioWorker, tablespace);
		blks[tablespace] = new BlockStream(tablespace, blockBuffer[tablespace]);
		ulog[tablespace] = new RecoveryLogManager(globalIO,tablespace);
		// Set the BlockStream as an observer of the block change events generated by 'getnextblk' in MappedBlockbuffer
		blockBuffer[tablespace].addBlockChangeObserver(blks[tablespace]);
	}
	
	public synchronized void reInitLog(GlobalDBIO globalIO, int tablespace) throws IOException {
		ulog[tablespace].getLogToFile().initializeLogFileSequence();
	}
	/**
	...
	*/
	private synchronized int findOrAddBlock(long tbn) throws IOException {
		...
		if(blks[tblsp].getBlockAccessIndex() != null ) {
			if(blks[tblsp].getBlockAccessIndex().getBlockNum() != tbn) {
				blks[tblsp].getBlockAccessIndex().decrementAccesses();
			} else {
				blks[tblsp].getBlockAccessIndex().byteindex = 0;
				return tblsp;
			}
		}
		blks[tblsp].setBlockAccessIndex(blockBuffer[tblsp].findOrAddBlock(tblk).get());
		if( DEBUG )
	...
	public synchronized void deallocOutstanding(long pos) throws IOException {
		...
		if( blks[tablespace].getBlockAccessIndex().getBlockNum() == pos ) {
			...
			if( DEBUG )
				System.out.printf("%s.deallocOutstanding current %s%n",this.getClass().getName(),GlobalDBIO.valueOf(pos));
		} else {
	 ...
	 * @throws IOException 
	 */
	public synchronized void commitBufferFlush(IOWorker[] ioWorker) throws IOException {
